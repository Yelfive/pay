...
class AopClient
{
    ...
    protected function curl($url, $postFields = null)
    {
        ...
        if (curl_errno($ch)) {
            ...
            if ($httpStatusCode == 302) {
                // Returns an array if its a redirect
                $response = [
    ...
    /**
     * 页面提交执行方法
     * @param AliPayTradeWapPayRequest $request ：跳转类接口的request; $httpmethod 提交方式。两个值可选：post、get
    ...
    public function execute($request, $authToken = null, $appInfoAuthtoken = null)
    {
        ...
        try {
            ...
            // Return if it's an array, which means its a redirect
        ...
        if (method_exists($request, "getNeedEncrypt") && $request->getNeedEncrypt()) {
            if ("json" == $this->format) {
                $response = $this->encryptJSONSignSource($request, $response);

                // 将返回结果转换本地文件编码
                $r = iconv($this->postCharset, $this->fileCharset . "//IGNORE", $response);
                $respObject = json_decode($r);
            } else {
                $response = $this->encryptXMLSignSource($request, $response);
